...
class PageController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $new_phim = phim::where('batdau', '<', $currentDate)->where('ketthuc', '>', $currentDate)->get();
    ...
    public function postChiTietLichSu(Request $req)
    {
        ...
        foreach ($ve as $v) {
            ...
            $ten_loai_ghe[] = ($temp->first()->tenloai);
        }
        ...
        //dd($suatchieu->first()->);
        return view('page.chitietlichsu', compact('ve', 'hoadon', 'phim', 'rap', 'ten_loai_ghe', 'soghe', 'khunggio', 'suatchieu'));
    }
    ...
    public function postSuaKhuyenmai(Request $req)
    {
        $this->validate($req,
         ...
         //dd($req['mgiamgia']);
        if ($req['id'] > 0) {
            $obj_user = khuyen_mai::find($req['id']);
    ...
    public function postSuaLoaiphong(Request $req)
    {
        ...
        //dd($req);
        //$obj_user = loai_phong::find($req['mtenloai']); // Problem: the 'loai_phong' table doesn't contain id column ???
        //$obj_user->tenloai = $req['mtenloai'];
        //$obj_user->gia = $req['mgia'];

        //$obj_user->save();
        DB::table('loai_phong')->where('tenloai',$req['mtenloai'])->update(['gia'=>$req['mgia']]);
        return redirect()->back()->with('success', 'Điều chỉnh thành công');
    }
    ...
    public function postSignin(Request $req)
    {
        ...
        $e = $req->email;
        $p = $req->password;
        $credentials = array('email' => $e, 'password' => $p);
        if (Auth::attempt($credentials)) {
            $data = $req->session()->all();
            $user = User::where('email', $e)->get();
    ...
    {
        $currentDate = Carbon\Carbon::now()->toDateString();
        $phim = phim::where('batdau', '<', $currentDate)->where('ketthuc', '>', $currentDate)->get();
    ...
    public function postThanhToan(Request $req)
    {
        ...
        $tongcong = (int)explode(' ', $tongcong)[0];
        if (Auth::user()) {
            //HOA DON
            $user_id = Auth::user()->id;
            $obj_user = User::find($user_id);
